...
abstract class Emp
{
	...
	Emp()
	{
		try
		{
			...
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/employees","root","mysql1234");
			PreparedStatement stmt=(PreparedStatement) con.prepareStatement("insert into mysqlconnection values(?,?,?,?)");
	...
	public static  void display()
	{
		try
		{
			...
			ResultSet rs =stmt.executeQuery("select * from mysqlconnection");
			while(rs.next())
			{
	...
	public static  void riseSalary()
	{
		try
		{
			Class.forName("com.mysql.jdbc.Driver"); // mysql 
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/employees","root","mysql1234");
			PreparedStatement stmt=(PreparedStatement) con.prepareStatement("update mysqlconnection set name  = ? where name =? ");
			Scanner sc=new Scanner(System.in);
	...
	public static  void deleteemp()
	{
		...
		try
		{
			Class.forName("com.mysql.jdbc.Driver"); // mysql 
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/employees","root","mysql1234");
			PreparedStatement stmt=(PreparedStatement) con.prepareStatement("delete from mysqlconnection where name =? ");
			Scanner sc=new Scanner(System.in);
